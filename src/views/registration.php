            ...
            <div class="content">
                <div style="text-align:center;" class="login">
                    <form action="login" method="post" enctype="multipart/form-data">
                        <h2>Założyć konto:</h2></br>
                        <p>E-mail: </p><input type="text" name="email"></br>
                        <?php if(isset($regResult) && $regResult==1) echo '<span style="color:red;">Email pole jest puste</span>'; ?>
                        <p>Login: </p><input type="text" name="login"></br>
                        <?php if(isset($regResult) && $regResult==2) echo '<span style="color:red;">Login pole jest puste</span>'; ?>
                        <p>Hasło: </p><input type="password" name="password"></br>
                        <?php if(isset($regResult) && $regResult==3) echo '<span style="color:red;">Hasło pole jset puste</span>'; ?>
                        <p>Powtórz hasło: </p><input type="password" name="r_password"></br>
                        <?php if(isset($regResult) && $regResult==4) echo '<span style="color:red;">Powtórz hasło jest puste</span>'; ?>
                        <?php if(isset($regResult) && $regResult==5) echo '<span style="color:red;">Pola hasło i powtórz hasło się nie zgadzają</span>'; ?>
                        <?php if(isset($regResult) && $regResult==6) echo '<span style="color:red;">Takie konto już istnieje</span>'; ?>
                        </br><input type="checkbox" name="register" checked="checked" style="opacity:0; position:absolute; left:9999px;">
                        <input type="submit" value="Załóż" name="submit"></br>
                    </form>
                    <a href="/galeria_login">Zalogować się</a></br>
                </div>
            </div>
